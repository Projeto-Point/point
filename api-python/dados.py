...
while verificaLogin == False:
    ...
    nomeFuncionario=cursor.execute(
        ("SELECT nome FROM Funcionario WHERE email = '{}' and senha = '{}'").format(login, senha))
...
#while verificaLogin == False:
   ...
   # login = input('Bem vindo ao Point! \nDigite o login do funcionário: ')
    ...
    # Executa o comando no banco que foi conectado
   # verificaLogin = cursor.execute(
     #   ("SELECT email, senha FROM Funcionario WHERE email = '{}' and senha = '{}'").format(login, senha))
   # nomeFuncionario = cursor.execute(
       # ("SELECT nome FROM Funcionario WHERE email = '{}' and senha = '{}'").format(login, senha))
...
#while cpu_percent() > 80.00:
        ...
        #mensagem1 = {"text": "A CPU do funcionario " +
               #      str(nomeFuncionario) + " está acima de 80%!"}
...
#while virtual_memory().percent > 85.00:
       ...
       # mensagem2 = {"text": "A memória Ram do funcionario " + str(
        #    nomeFuncionario{ + " está acima de 85%! " + "com: " + str(virtual_memory().percent) + "%"}
...
#conexao = pymysql.connect(db='bd_point_python', user='aluno', passwd='sptech')
#cursor = conexao.cursor()
#verificarCadastro = cursor.execute(
#    ("SELECT nomeMaquina FROM Maquina WHERE nomeMaquina = '{}'").format(nome))
...
if verificarCadastro != 0:
   ...
   # cursor = conexao.cursor()
    ...
    cursor.execute(
        (f"""SELECT idFuncionario FROM Funcionario WHERE email = '{login}' and senha = '{senha}'"""))

    fkFuncionario = cursor.fetchall()[0][0]

    cursor.execute(
        f"""INSERT INTO Maquina VALUES (null, '{platform.system()}', '{platform.node()}', 'Sistema', {fkFuncionario})""")

    cursor.execute(
        (f"""SELECT idMaquina FROM Maquina WHERE nomeMaquina = '{nome}'"""))
    ...
    cursor.execute(
        f"INSERT INTO Componente (idComponente, fkMaquina, tipo) VALUES (1, {id}, 'CPU')")

    cursor.execute(
        f"""INSERT INTO Componente (idComponente, fkMaquina, tipo) VALUES (2, {id}, 'RAM')""")

    cursor.execute(
        f"""INSERT INTO Componente (idComponente, fkMaquina, tipo) VALUES (3, {id}, 'Disco')""")

    cursor.execute(
        f"INSERT INTO Atributo (atributo, valor, unidadeMedida, fkMaquina, fkComponente) VALUES (null, {memoriaTotal}, 'GB', {id}, 2)")
    cursor.execute(
        f"INSERT INTO Atributo (atributo, valor, unidadeMedida, fkMaquina, fkComponente) VALUES (null, {discoTotal}, 'GB', {id}, 3)")
...
while True:
    ...
    agora_datetime = datetime.datetime.strptime(
    ...
    try:
        cursor.execute(
            f"INSERT INTO Registro (valor, unidadeMedida, dataEhora, fkComponente, fkMaquina) VALUES ({cpu_percent(interval=0.1)}, '%', NOW(), 1, {id})")

        cursor.execute(
            f"INSERT INTO Registro (valor, unidadeMedida, dataEhora, fkComponente, fkMaquina) VALUES ({virtual_memory().percent}, '%', NOW(), 2, {id})")
